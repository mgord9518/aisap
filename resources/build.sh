...
CC=gcc go build -ldflags '-s -w -linkmode external -extldflags=-static' -o '../../AppDir/usr/bin'
[ $? -ne 0 ] && exit $?
cd ../..
...
cd cmd/aisap
go mod tidy
CC=gcc GOARCH=arm64 go build -ldflags '-s -w -linkmode external -extldflags=-static' -o '../../AppDir/usr.aarch64/bin'
cd ../..
